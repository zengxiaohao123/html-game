...
- [x] **游戏本体已实现并部署（可试玩）**
  ...
  - **大改版(2026-09-03，本轮)**：
    - 队友占位：新增 `js/data.js` 的 `ALLIES`（夏阳·火 / 陆悠悠·风）+ `CHARACTERS/getChar/getTeamChars`；`G.team` 默认 `['pro','xiayang','luyouyou']` 三人（仅测试用，数值未按规格书平衡）。
    - **移动逻辑**：探索朝障碍/地图外移动→只转向+不消耗行动力（返还）；战斗朝障碍/敌人格移动→只转向+计为本回合已移动（不返还）。探索敌人格仍直接进入战斗（敌人格非实体）。
    - **攻击朝向**：方向性技能（斩击等 front/frontline）必须有该方向敌人才能释放；移动前主动使用失败→不视为使用+文字提醒；移动后自动释放失败→该角色本回合跳过。
    - **UI区域分工**：左=行动记录（增宽、战斗开始/结束/睡觉时清空）；中=地图（变窄、滚轮缩放+拖拽平移、不规则形状+随机出生）；右=信息区（只显示被点格信息+「前往」按钮+事件分歧选项双击确认，不负责操作提醒）；底部剧情区（只承载文学剧情，进入战斗/睡觉清空）战斗时替换为**角色技能区**（3角色卡 F1/F2/F3、属性、技能 1/2/3 单击选/再点=主动用、天赋小标签点击弹窗、右侧技能详情）。
    - **地图**：生成不规则连通地块（生长法，非100%正方形）+ 主角随机出生点。
    - **指示器**：敌人红色小三角朝向；选中技能生效范围=浅橙色；敌人本回合要攻击的范围=浅红色（均仅视觉）。
    - 任务/编队/角色/背包/设置页改为**全屏面板**（`.modal.full`，94vh）。
