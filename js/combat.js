...
function charAtk(charKey){ const c=getChar(charKey); const base=charBaseAtk(charKey); let a=base; for(const p of c.passives){ if(p.scal && p.scal.atk){ a += tierValue(p, entryLevel(charKey,p), 'atk'); } } if(charKey==='pro'){ for(const k in ALLIES){ for(const p of (ALLIES[k].passives||[])){ if(p.scal&&p.scal.pro) a+=tierValue(p, entryLevel(k,p),'pro'); } } } if(combatState&&combatState.ally[charKey]){ const sts=combatState.ally[charKey].statuses; if(sts.atkUp) a+=Math.round(base*0.25); a += (combatState.ally[charKey].flatAtk||0); } return Math.round(a); }
function heroineMaxHp(){ return heroDisplayMaxHp(); }
function heroDodgeRate(){ if(G&&G.team&&G.team.indexOf('luyouyou')>=0){ const c=ALLIES.luyouyou; const fl=c.passives.find(p=>p.id==='flutter'); return fl? vTier(fl,'dodge',entryLevel('luyouyou',fl)) : 0; } return 0; }
...
function bindCombatGo(x,y){ const cs=combatState; const go=$('#goBtn'); if(cs.playerMoved){ go.style.display='none'; return; } const ok=moveCostFor(x,y)!==null; if(!ok){ go.style.display='none'; return; } go.style.display='block'; go.disabled=false; go.textContent='前往'; go.onclick=confirmCombatMove; }
function confirmCombatMove(){ const cs=combatState; if(!cs) return; if(cs.playerMoved||!cs.pendingTarget) return; const {x,y}=cs.pendingTarget; if(moveCostFor(x,y)===null){ log('该格无法前往。'); $('#goBtn').style.display='none'; return; } const bindS=hasStatus(cs.ally.pro.statuses,'bind'); if(bindS){ log('【束缚】使你无法移动，本回合行动结束。'); cs.playerMoved=true; autoCastAll(); if(!combatState) return; endPlayerPhase(); return; } const ox=cs.hero.x, oy=cs.hero.y; cs.hero.facing=dirToFacing(x-ox, y-oy); cs.hero.x=x; cs.hero.y=y; consumeVehicleForMove(); cs.playerMoved=true; autoCastAll(); if(!combatState) return; endPlayerPhase(); }
...
function resolveSkill(charKey, skill, manual){
  ...
  for(const enemy of targets){
    ...
    if(critThis) dmg*=2;
    const critTxt=critThis&&!forceCrit?'<span class="crit-hint">暴击！</span>':'';
    const dm=damageEnemy(enemy,dmg,finalType,charKey);
...
function damageHero(dmg, type){ const cs=combatState; const pro=getChar('pro'); const real=(type==='true'||type==='real');
  ...
  if(!real && flP && G.team.indexOf('luyouyou')>=0){ const dr=vTier(flP,'dodge',entryLevel('luyouyou',flP)); if(Math.random()*100<dr){ const hv=vTier(flP,'combat',entryLevel('luyouyou',flP)); const nx=Math.min(heroineMaxHp()||100,(G.hero.hp||0)+hv); const got=nx-G.hero.hp; G.hero.hp=nx; cs.hero.hp=nx; log(`【闪避】触发，本次伤害降为0${got>0?`，回复 ${got} 点生命。`:''}`); return; } }
...
function charAttrsHTML(key){ if(key==='pro'){ const h=G.hero; return `<span class="attr"><b>攻击</b> ${charAtk('pro')}</span><span class="attr"><b>闪避率</b> ${heroDodgeRate()}%</span><span class="attr"><b>生命</b> ${Math.round(combatState.hero.hp)}/${heroineMaxHp()}</span><span class="attr"><b>防御</b> ${totalHeroDefense()}</span><span class="attr"><b>暴击率</b> ${charCritRate(key)}%</span><span class="attr"><b>逃跑速度</b> ${h.escapeSpeed}</span>`; } const c=getChar(key); return `<span class="attr"><b>攻击</b> ${charAtk(key)}</span><span class="attr"><b>暴击率</b> ${charCritRate(key)}%</span>`; }
...
function heroInfoHTML(){ const cs=combatState; return `<b>主角</b><br>攻击 ${charAtk('pro')} · 防御 ${totalHeroDefense()} · 生命 ${Math.round(cs.hero.hp)}/${heroineMaxHp()}${cs.hero.shield?`<br>护盾 ${Math.round(cs.hero.shield)}`:''}<br><div class="sec">状态</div>${statusBarHTML(cs.ally.pro.statuses,cs.field)}`; }
...
function showCombatEndPopup(title, lines, thenGameOver){ openModal(title, `<div style="min-width:250px"><p style="font-size:17px;font-weight:bold;margin-bottom:6px">${title}！</p><div style="font-size:14px;line-height:1.6;margin-bottom:10px">${lines.map(l=>`<div>${l}</div>`).join('')}</div><div style="text-align:center"><button class="mbtn" id="ceConfirm">确认</button></div></div>`, 'small'); const btn=document.getElementById('ceConfirm'); if(btn) btn.onclick=()=>{ closeModal(); clearLog(); for(const l of lines) log(l); refreshHUD(); if(thenGameOver) showGameOver(); }; }
