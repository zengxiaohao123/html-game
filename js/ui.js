...
function openSettings(){ const lbl = combatState? '存档（回本次战斗开始时）' : (eventState? '存档（回本次事件开始时）' : '存档'); openModal('设置', `<div style="display:flex;gap:14px;flex-wrap:wrap"><button class="mbtn small" onclick="saveMenuOpen()">${lbl}</button><button class="mbtn small" onclick="openReadSave()">读档</button><button class="mbtn small" onclick="closeModal();backToMenu()">返回主界面（不存档）</button></div>`, 'full'); }
...
function renderInventory(){ const keys=Object.keys(G.inventory).filter(k=>k!=='coin' && (G.inventory[k]||0)>0); const tiles=keys.map(k=>{ const n=G.inventory[k]; const useBtn = (!combatState && !eventState && itemUsable(k)) ? `<button class="mbtn tiny invUse" onclick="useInvItem('${k}')">使用</button>` : ''; return `<div class="itile"><div class="iname craftlink" data-key="${k}">${itemName(k)}</div><div class="icount">×${n}</div>${useBtn}</div>`; }).join(''); openModal('背包', `<div class="invbar"><span class="invtitle">随身物品</span><span class="invcoin">金币 <b>${G.inventory.coin}</b></span></div>${invMsg?`<div class="shopmsg">${invMsg}</div>`:''}<div class="vgrid inv">${tiles||'<span class="stempty">背包空空如也</span>'}</div>`, 'full', {replace:true}); }
...
function renderCharacters(){ const keys=['pro',...Object.keys(ALLIES)]; const tabs=keys.map(k=>`<button class="ctab ${k===charPageKey?'on':''}" data-k="${k}">${getChar(k).name}</button>`).join(''); openModal('角色', `<div class="ctabs">${tabs}</div><div id="charPageBody">${charPageBody(charPageKey)}</div>`, 'full'); $('#modalBody').querySelectorAll('.ctab').forEach(b=>b.onclick=()=>{ charPageKey=b.dataset.k; renderCharacters(); }); }
function charPageBody(key){ const c=getChar(key); const bondHTML = key==='pro' ? '' : `<span>羁绊等级 <b>${G.bonds&&G.bonds[key]?G.bonds[key].level:1}</b></span>`; const attrs = key==='pro' ? `<span>攻击 <b>${G.hero.atk}</b></span><span>最大生命 <b>${G.hero.maxHp}</b></span><span>防御 <b>${G.hero.def}</b></span><span>逃跑速度 <b>${G.hero.escapeSpeed}</b></span><span>健康 <b>${G.hero.health}</b></span><span>行动力上限 <b>${G.hero.apCap}</b></span>` : `<span>攻击 <b>${c.atk}</b></span><span>属性 <b>${c.element?ELEM[c.element].zh:'无'}</b></span>${bondHTML}`; const talents=c.passives.map(p=>{ const name=p.scal? talentDisplayName(key,p) : p.name; const desc=p.scal? lvDescText(p, entryLevel(key,p)) : terms(p.desc); return `<div class="charTalent"><span class="cat talent">天赋</span>${name}：${desc}</div>`; }).join(''); const skills=c.skills.map(s=>`<div class="charSkill"><span class="cat ${s.kind==='attack'?'attack':'support'}">${s.kind==='attack'?'攻击':'辅助'}</span>${skillDisplayName(key,s)}${c.selectedSkillIds.includes(s.id)?' <span class="carry">[携带]</span>':''}：${describeSkill(key,s)}</div>`).join(''); return `<div class="charHead">${c.name}${c.element?`（<span class="${ELEM[c.element].c}">${ELEM[c.element].zh}</span>）`:''}</div><div class="statGrid">${attrs}</div><div class="sec"><b>天赋</b></div>${talents}<div class="sec"><b>技能</b>（战斗中可携带至多3个）</div>${skills}<div style="margin-top:14px"><button class="mbtn" onclick="openSkillManager('${key}')">调整携带技能</button></div>`; }
let skillPickSel={};
function openSkillManager(key){ skillPickSel[key]=getChar(key).selectedSkillIds.slice(); renderSkillManager(key); }
function renderSkillManager(key){ const c=getChar(key); const sel=skillPickSel[key]; const skills=c.skills.map(s=>{ const on=sel.includes(s.id); return `<div class="skillpick ${on?'sel':''}" onclick="toggleSkillPick('${key}','${s.id}')">${on?'☑':'☐'} <b>${skillDisplayName(key,s)}</b>：${describeSkill(key,s)}</div>`; }).join(''); openModal(`调整携带技能 · ${c.name}`, `<p style="font-size:14px;margin-bottom:8px">至多选择 3 个技能（当前 ${sel.length}/3，天赋不计）。</p>${skills}<div class="btn-row" style="margin-top:10px"><button class="mbtn small" onclick="saveSkillManager('${key}')">保存</button><button class="mbtn small" onclick="openCharacters()">取消</button></div>`); }
window.toggleSkillPick=function(key,id){ const sel=skillPickSel[key]; const i=sel.indexOf(id); if(i>=0) sel.splice(i,1); else { if(sel.length>=3){ alert('至多携带 3 个技能。'); return; } sel.push(id); } renderSkillManager(key); };
...
function renderFormation(){ const team=G.team.slice(); const rows=team.map((k,i)=>{ const bond = k!=='pro' ? ` <span class="fbond">羁绊 ${G.bonds&&G.bonds[k]?G.bonds[k].level:1} 级</span>` : ''; return `<div class="frow"><span class="fpos">${i+1}号位</span><span class="fname">${getChar(k).name}${bond}</span>${i>0?`<button class="mbtn small" onclick="moveSlot(${i},-1)">上移</button>`:''}${i<team.length-1?`<button class="mbtn small" onclick="moveSlot(${i},1)">下移</button>`:''}</div>`; }).join(''); openModal('编队', `<p style="font-size:15px">调换队伍一二三号位（影响战斗自动释放顺序）。</p>${rows}`, 'full'); }
window.moveSlot=function(i,dir){ const t=G.team.slice(); const j=i+dir; if(j<0||j>=t.length) return; const tmp=t[i]; t[i]=t[j]; t[j]=tmp; G.team=t; renderFormation(); };
function openTasks(){ openModal('任务', `<p style="font-size:15px">任务记录：</p><p style="font-size:15px;margin:8px 0">· 存活下去（进行中）</p><p style="font-size:15px;margin:8px 0">· 探索第 ${G.day} 天的新地图（进行中）</p>`); }
...
document.addEventListener('click',ev=>{ const st=ev.target.closest('.stchip'); if(st){ const rounds=st.textContent.match(/·(\d+)回合/); openPopoverNear(st, `<b>${st.dataset.name}</b>${rounds?`（${rounds[1]}回合）`:''}<br>${st.dataset.desc||''}`); return; } const tg=ev.target.closest('.talentTag'); if(tg){ const owner=tg.dataset.k; const c=getChar(owner); const t=c.passives[+tg.dataset.i]; if(t){ const name=t.scal? talentDisplayName(owner,t) : t.name; const desc=t.scal? lvDescText(t, entryLevel(owner,t)) : t.desc; openPopoverNear(tg, `<b>${name}</b><br>${desc}`); } return; } const cl=ev.target.closest('.craftlink'); if(cl){ const key=cl.dataset.key; openPopoverNear(cl, `<b>${itemName(key)}</b><br>${itemDetailHTML(key)}`); return; } $('#popover').style.display='none'; });
document.addEventListener('keydown', ev=>{ if(ev.key!=='Escape') return; ev.preventDefault(); if($('#menuOverlay').classList.contains('show') || $('#gameoverOverlay').classList.contains('show')) return; if($('#modalOverlay').classList.contains('show')){ modalBack(); return; } if(G) openSettings(); });