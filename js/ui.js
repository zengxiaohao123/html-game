...
function renderCharacters(){ const keys=['pro',...Object.keys(ALLIES)]; const tabs=keys.map(k=>`<button class="ctab ${k===charPageKey?'on':''}" data-k="${k}">${getChar(k).name}</button>`).join(''); openModal('角色', `<div class="ctabs">${tabs}</div><div id="charPageBody">${charPageBody(charPageKey)}</div>`, 'full', {replace:true}); $('#modalBody').querySelectorAll('.ctab').forEach(b=>b.onclick=()=>{ charPageKey=b.dataset.k; renderCharacters(); }); interactInit(); }
function charPageBody(key){
  const c=getChar(key); const isPro = key==='pro';
  const b = isPro? null : getBond(key);
  const headText = isPro ? c.name : `${c.name}${c.element?`（<span class="${ELEM[c.element].c}">${ELEM[c.element].zh}</span>）`:''}<span class="cbond">羁绊 ${b.level} 级 · 好感度 ${b.affinity}</span>`;
  const nav=['skills','技能展示','carry','携带技能调整','interact','交互','bond','羁绊等级效果','story','故事'];
  let navHtml=''; for(let i=0;i<nav.length;i+=2){ const id=nav[i],label=nav[i+1]; navHtml+=`<button class="csub ${charPageTab===id?'on':''}" onclick="setCharPageTab('${id}')">${label}</button>`; }
  ...
  return `<div class="charHead">${headText}</div><div class="csubnav">${navHtml}</div><div class="charPageSub">${body}</div>`;
}
window.setCharPageTab=function(id){ if(!['skills','carry','interact','bond','story'].includes(id)) return; charPageTab=id; renderCharacters(); };
function charSkillsTab(key,c){ c=c||getChar(key); const attrs= key==='pro'? `<span>攻击 ${R(charAtk('pro'))}</span><span>最大生命 ${R(heroDisplayMaxHp())}</span><span>防御 ${R(heroDisplayDef())}</span><span>逃跑速度 ${R(G.hero.escapeSpeed)}</span><span>健康 ${R(G.hero.health)}</span><span>行动力上限 ${R(G.hero.apCap)}</span>` : `<span>攻击 ${R(charAtk(key))}</span><span>属性 ${c.element?ELEM[c.element].zh:'无'}</span>`; const talents=c.passives.map(p=>{ const name=p.scal? talentDisplayName(key,p):p.name; const desc=p.scal? lvDescText(p,entryLevel(key,p)) : terms(p.desc); return `<div class="charTalent"><span class="cat talent">天赋</span>${name}：${desc}</div>`; }).join(''); const skills=c.skills.map(s=>`<div class="charSkill"><span class="cat ${s.kind==='attack'?'attack':'support'}">${s.kind==='attack'?'攻击':'辅助'}</span>${skillDisplayName(key,s)}${c.selectedSkillIds.includes(s.id)?' <span class="carry">[携带]</span>':''}：${describeSkill(key,s)}</div>`).join(''); return `<div class="statGrid">${attrs}</div><div class="sec"><b>天赋</b></div>${talents}<div class="sec"><b>技能</b>（战斗中可携带至多3个）</div>${skills}`; }
...
function interactButtons(key){ if(key==='xiayang'){ return [['chat','聊天（成功率 50%）'],['feed','投喂'],['gift','送礼']]; } if(key==='luyouyou'){ const st=lyChatState(); return [['chat',`聊天（成功率 ${st.cur}%）`],['gift','送礼']]; } return []; }
function renderInteractBody(key){
  ...
  if(giftOpenKey===key) html = `<div class="gift-overlay" id="giftOverlay">${renderGiftGrid(key)}</div>` + html;
  wrap.innerHTML=html;
...
/* 聊天 */
function lyChatState(){ G.records=G.records||{}; if(!G.records.lychat) G.records.lychat={day:0,base:0,cur:0}; const s=G.records.lychat; const day=G.day||1; if(s.day!==day){ s.day=day; s.base=Math.round((Math.random()*80-20)*10)/10; s.cur=s.base; } return s; }
function interactChat(key){ const c=getChar(key); if((G.hero.actionPoint||0)<1){ interactSay(key,`你的行动力不足，无法与 ${c.name} 聊天。（聊天需消耗 1 行动力）`); return; } G.hero.actionPoint-=1; refreshHUD(); if(key==='xiayang'){ if(Math.random()<0.5){ gainAffinity(key,1); interactSay(key,`${c.name}：你讲了个烤熊掌的笑话，夏阳先是愣了一下，随后笑出了声。……你俩相谈甚欢。<span class="lvlup">好感度+1</span>（消耗 1 行动力）`); } else { interactSay(key,`你聊起路上的见闻，夏阳却只是「嗯嗯」地点着头，明显兴致缺缺。<span class="lvlup">好感度+0</span>（消耗 1 行动力）`); } } else if(key==='luyouyou'){ const st=lyChatState(); const ok = st.cur>=0 && Math.random()*100<st.cur; if(ok){ const add=Math.round((2+Math.random()*2)*10)/10; st.cur+=add; gainAffinity(key,1); interactSay(key,`${c.name}：哈哈，你说话真有意思，我很受用。<span class="lvlup">好感度+1</span>（本日聊天成功率 +${add}%，消耗 1 行动力）`); } else { const sub=Math.round((2+Math.random()*2)*10)/10; st.cur-=sub; interactSay(key,`${c.name}：嗯……这句就没那么有趣了。我再看下路线。<span class="lvlup">好感度+0</span>（本日聊天成功率 -${sub}%，消耗 1 行动力）`); } } }
/* 投喂（仅夏阳）：自动消耗回复量最少的食物 */
...
function decorateGiftCells(){ const box=$('#giftOverlay'); if(!box) return; if(giftCooldownLeft(giftOpenKey)>0) return; box.querySelectorAll('.gift-cell').forEach(c=>c.onclick=()=>{ const k=c.dataset.k; giftSelItem = (giftSelItem===k)? null : k; renderInteractBody(giftOpenKey); decorateGiftCells(); }); }
function confirmGift(){ const key=giftOpenKey; if(!key||!giftSelItem) return; if(giftCooldownLeft(key)>0) return; const it=giftSelItem; const lv=itemLoveLevel(key,it); const L=(ITEM_LOVE[key]||{}); let delta=0, talk=''; if(lv===0){ delta=-1; talk=GIFT_TALK[key].lv0; } else if(lv===1){ delta=1; talk=GIFT_TALK[key].lv1; } else if(lv===2){ delta=L.two[it]; talk=GIFT_TALK[key].lv2; } else { delta=(L.three[it]||0)+5; talk=(GIFT_TALK[key]['lv3_'+it])||''; } G.inventory[it]--; G.records=G.records||{}; if(!G.records.giftDay) G.records.giftDay={}; G.records.giftDay[key]=G.day||1; giftOpenKey=null; giftSelItem=null; if(delta!==0) gainAffinity(key,delta); refreshHUD(); interactSay(key, `${talk} <span class="lvlup">好感度${delta>0?'+'+delta:delta}</span>`); }
function closeGift(){ if(giftOpenKey==null) return; giftOpenKey=null; giftSelItem=null; renderInteractBody(charPageKey); }
