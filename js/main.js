...
window.addEventListener('load',()=>{
  try { bindTooltip(); } catch(e){ console.warn('bindTooltip fail',e); }
  try { switchMode('story'); } catch(e){ console.warn('switchMode fail',e); }
  try {
    const saves = loadSaves();
    if(saves[1]) loadGame(1);
  } catch(e){
    console.warn('旧存档不兼容，跳过自动读档。错误:', e.message);
    localStorage.removeItem('yysw_saves');
  }
